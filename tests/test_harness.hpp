...
class TestFixture {
 public:
  ...
  [[nodiscard]] std::optional<art2img::Tile> find_non_empty_tile(
      const std::string& art_name = "tiles000.art");

  /// Find first non-empty tile in given ART file
  [[nodiscard]] static std::optional<art2img::Tile> find_non_empty_tile(
      const art2img::ArtFile& art);
