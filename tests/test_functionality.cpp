...
TEST_CASE("Default Duke3D palette matches external PALETTE.DAT output") {
    ...
    for (const auto& art_path : art_files) {
        ...
        REQUIRE(explicit_extractor.load_palette_from_memory(palette_bytes.data(), art2img::Palette::SIZE));
        ...
        for (auto tile_index : sample_indices(tile_count)) {
            auto default_tile = default_extractor.extract_tile(tile_index);
            auto explicit_tile = explicit_extractor.extract_tile(tile_index);
...
TEST_CASE("Extractor produces both PNG and TGA payloads in memory") {
    ...
    auto png_result = extractor.extract_tile(0);
...
TEST_CASE("CLI help documents Duke Nukem 3D palette usage") {
    ...
    CHECK(help_text.find("Duke Nukem 3D palette") != std::string::npos);
