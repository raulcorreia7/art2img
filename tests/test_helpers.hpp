...
/// @brief Central test helper functions for consistent test directory management
...
inline std::filesystem::path get_integration_test_dir(const std::string& test_name) {
...
inline std::filesystem::path get_unit_test_dir(const std::string& category, const std::string& test_name) {
...
inline void ensure_test_output_dir(const std::filesystem::path& dir) {
    ...
    if (ec) {
        throw std::runtime_error("Failed to create test output directory: " + dir.string() + " - " + ec.message());
    }
}
...
inline void cleanup_test_output_dir(const std::filesystem::path& dir) {
    ...
    if (ec) {
        std::cerr << "Warning: Failed to clean up test directory: " << dir.string() << " - " << ec.message() << std::endl;
    }
}
...
inline void debug_test_output_dir(const std::string& context) {
    std::cout << "Test output directory (" << context << "): " << get_test_output_dir() << std::endl;
}
