...
public:
    ...
    static std::vector<std::byte> create_minimal_art_file(
        ...
        const std::size_t pixel_count = static_cast<std::size_t>(tile_width) * static_cast<std::size_t>(tile_height);
    ...
    static std::vector<std::byte> create_multi_tile_art_file(
        ...
        for (std::uint32_t i = 0; i < tile_count; ++i) {
            const std::size_t pixel_count = static_cast<std::size_t>(widths[i]) * static_cast<std::size_t>(heights[i]);
...
TEST_SUITE("ART Loader") {
    ...
    TEST_CASE("TileAnimation picanm conversion") {
        ...
        CHECK(anim.type == static_cast<art2img::TileAnimation::Type>((original_picanm >> 6) & 0x03));
        CHECK(anim.speed == ((original_picanm >> 24) & 0x0F));
        CHECK(anim.y_center_offset == static_cast<std::int8_t>((original_picanm >> 8) & 0xFF));
        CHECK(anim.x_center_offset == static_cast<std::int8_t>((original_picanm >> 16) & 0xFF));
        
        const std::uint32_t converted_picanm = anim.to_picanm();
        // Note: Some bits may be lost in conversion due to field size limits
        CHECK((converted_picanm & 0x3F) == (original_picanm & 0x3F)); // frame_count
        CHECK(((converted_picanm >> 6) & 0x03) == ((original_picanm >> 6) & 0x03)); // type
        CHECK(((converted_picanm >> 24) & 0x0F) == ((original_picanm >> 24) & 0x0F)); // speed
    }
...
TEST_SUITE("ART Error Handling") {
    ...
    TEST_CASE("Invalid tile dimensions") {
        ...
        // This test would require creating a custom ART file with invalid dimensions
        // which is complex to do without the helper functions
...
TEST_SUITE("Sidecar File Discovery") {
    ...
    TEST_CASE("load_lookup_data from span") {
        const std::filesystem::path lookup_path = TEST_ASSET_SOURCE_DIR "/LOOKUP.DAT";
    ...
    TEST_CASE("load_lookup_data - invalid size") {
        ...
        REQUIRE(!result.has_value());
        CHECK(result.error().code == art2img::errc::invalid_art);
        CHECK(result.error().message.find("must be at least 256") != std::string::npos);
    }
}
...
TEST_SUITE("Real ART Files") {
    ...
    TEST_CASE("Load TILES000.ART") {
        ...
        if (!result.has_value()) {
            ...
            std::cerr << "Error loading " << art_file << ": " << result.error().message << std::endl;
        }
        ...
        for (std::size_t i = 0; i < std::min<std::size_t>(5, art_data.tile_count()); ++i) {
            const auto tile = art_data.get_tile(i);
            REQUIRE(tile.has_value());
            CHECK(tile->is_valid());
            CHECK(tile->has_valid_pixel_data());
    ...
    TEST_CASE("Load TILES001.ART with palette hint") {
        const std::filesystem::path art_file = TEST_ASSET_SOURCE_DIR "/TILES001.ART";
        
        if (!std::filesystem::exists(art_file)) {
            return; // Skip test if file not found
        }
        
        auto result = art2img::load_art_bundle(art_file, art2img::PaletteHint::both);
