...
struct AnimationTestFixture {
    ...
    static std::vector<std::byte> create_animated_art_file(
        ...
        std::uint32_t picanm = 0x12345678) {
    ...
    static std::vector<std::byte> create_non_animated_art_file(
        ...
        std::uint16_t tile_width = 8,
        std::uint16_t tile_height = 8) {
        return create_animated_art_file(tile_start, tile_width, tile_height, 0); // picanm = 0 (no animation)
    }
...
TEST_SUITE("Animation Export Unit Tests") {
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "export_animation_data with animated tile") {
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "export_animation_data exclude non-animated") {
        ...
        // Export animation data
        auto export_result = art2img::export_animation_data(art_data, config_exclude);
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "image format awareness in INI output") {
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "disable image references in INI output") {
        ...
        // Export animation data
        auto export_result = art2img::export_animation_data(art_data, config_no_refs);
        REQUIRE(export_result.has_value());
        CHECK(ini_file_exists());
...
TEST_SUITE("Animation Export Integration Tests") {
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "legacy format compatibility") {
        ...
        CHECK(ini_content.find(std::to_string(static_cast<int>(anim.x_center_offset))) != std::string::npos);
        CHECK(ini_content.find(std::to_string(static_cast<int>(anim.y_center_offset))) != std::string::npos);
    }
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "real ART file animation detection") {
        ...
        for (std::size_t i = 0; i < total_count; ++i) {
            ...
            if (tile.has_value() && tile->animation.type != art2img::TileAnimation::Type::none) {
                animated_count++;
            }
        }
        
        INFO("Found ", animated_count, " animated tiles out of ", total_count, " total tiles");
...
TEST_SUITE("Animation Export Error Handling") {
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "file system permission errors") {
        ...
        auto export_result = art2img::export_animation_data(art_result.value(), config_readonly);
    ...
    TEST_CASE_FIXTURE(AnimationTestFixture, "invalid ART data") {
        ...
        std::vector<std::byte> corrupted_data = {std::byte{0x01}}; // Just version byte
