...
namespace
{
    ...
    std::vector<types::byte> create_test_binary_data()
    {
        ...
        const std::string pattern = "Hello, World! This is a test pattern for binary I/O.";
        for (char c : pattern)
        {
...
TEST_SUITE("io")
{
    ...
    TEST_CASE("read_binary_file - success")
    {
        ...
        CHECK(std::memcmp(read_data.data(), test_data.data(), test_data.size()) == 0);
    ...
    TEST_CASE("write_binary_file - success")
    {
        ...
        CHECK(std::memcmp(read_back.data(), test_data.data(), test_data.size()) == 0);
    ...
    {
        // Note: This test might not work on all systems due to different permission models
        // We'll test the error path by trying to write to a location that likely doesn't exist
        const auto impossible_path = std::filesystem::path("/impossible/path/test.dat");
    ...
    TEST_CASE("binary file round-trip")
    {
        ...
        CHECK(std::memcmp(read_data.data(), original_data.data(), original_data.size()) == 0);
