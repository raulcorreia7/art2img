...
TEST_CASE("Integration tests - Full pipeline") {
  ...
  SUBCASE("Memory vs file consistency") {
    ...
    auto file_result = file_extractor.extract_tile_png(0);
    auto memory_result = memory_extractor.extract_tile_png(0);
  ...
  SUBCASE("Batch extraction consistency") {
    ...
    auto png_results = extractor.extract_all_tiles_png();
  ...
  SUBCASE("Batch extraction BMP format") {
    ...
    for (const auto& result : bmp_results) {
      ...
      if (result.success) {
        ...
        if (result.image_data.empty()) {
          ...
          REQUIRE_GE(result.image_data.size(), 54);  // Minimum BMP header size
          CHECK_EQ(result.image_data[0], 'B');
          CHECK_EQ(result.image_data[1], 'M');
        }
      }
    }
    ...
    // Empty tile counts should match other formats
    // Note: There are 39 empty tiles in the test data
    CHECK_EQ(bmp_empty_count, 39);
  }
  ...
  SUBCASE("ImageView and ExtractorAPI consistency") {
    ...
    for (uint32_t i = 0; i < std::min(art_view.image_count(), static_cast<size_t>(5UL)); ++i) {
      ...
      auto extractor_result = extractor.extract_tile_png(i);

      // Extract via ImageView
      art2img::ImageView image_view{&art_view, i};
      auto image_data = image_view.extract_to_png(art2img::ImageWriter::Options());
  ...
  SUBCASE("ImageView and ExtractorAPI BMP consistency") {
    art2img::ExtractorAPI extractor;
    auto art_data = load_test_asset("TILES000.ART");
    auto palette_data = load_test_asset("PALETTE.DAT");
    ...
    for (uint32_t i = 0; i < std::min(art_view.image_count(), static_cast<size_t>(5UL)); ++i) {
      ...
      // Extract via ExtractorAPI
      auto extractor_result = extractor.extract_tile_bmp(i);
...
TEST_CASE("Integration tests - Performance and limits") {
  ...
  SUBCASE("Large batch extraction") {
    ...
    for (uint32_t i = 0; i < 10 && i < extractor.get_tile_count(); ++i) {
      ...
      auto result = extractor.extract_tile_png(tile_index);
      CHECK(result.success);
    }
  }
...
TEST_CASE("Integration tests - Animation data") {
  ...
  SUBCASE("Animation data consistency") {
    ...
    for (uint32_t i = 0; i < std::min(art_view.image_count(), static_cast<size_t>(50UL)); ++i) {
      art2img::ImageView image_view{&art_view, i};
  ...
  SUBCASE("ExtractionResult animation data") {
    ...
    for (uint32_t i = 0; i < std::min(results.size(), static_cast<size_t>(50UL)); ++i) {
