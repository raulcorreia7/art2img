...
TEST_CASE("Corrupted PALETTE.DAT file handling") {
    ...
    SUBCASE("Palette file with incorrect size") {
        // Create a copy with truncated data
        auto corrupted_data = std::vector<uint8_t>(valid_palette_data.begin(), valid_palette_data.begin() + 100);
...
TEST_CASE("Corrupted LOOKUP.DAT file handling") {
    ...
    SUBCASE("LOOKUP.DAT file with invalid indices") {
        ...
        CHECK_NOTHROW(extractor.load_art_file(filename)); // This should not crash even with invalid data
        CHECK(!extractor.load_art_from_memory(corrupted_data.data(), corrupted_data.size()));
    ...
    SUBCASE("LOOKUP.DAT file with truncated data") {
        // Create a copy with truncated data
        auto corrupted_data = std::vector<uint8_t>(valid_lookup_data.begin(), valid_lookup_data.begin() + 50);
        ...
        art2img::ExtractorAPI extractor;
        // LOOKUP.DAT is not directly loaded by ExtractorAPI, but we test it doesn't crash
        CHECK_NOTHROW(extractor.load_art_file(filename)); // This should not crash even with truncated data
        CHECK(!extractor.load_art_from_memory(corrupted_data.data(), corrupted_data.size()));
    ...
    SUBCASE("LOOKUP.DAT file with garbage data") {
        ...
        art2img::ExtractorAPI extractor;
        // LOOKUP.DAT is not directly loaded by ExtractorAPI, but we test it doesn't crash
        CHECK_NOTHROW(extractor.load_art_file(filename)); // This should not crash even with garbage data
    ...
    SUBCASE("Extremely large LOOKUP.DAT file") {
        ...
        CHECK(!extractor.load_art_from_memory(nullptr, 100 * 1024 * 1024)); // This should fail gracefully
...
TEST_CASE("Corrupted TABLES.DAT file handling") {
    ...
    SUBCASE("TABLES.DAT file with truncated data") {
        // Create a copy with truncated data
        auto corrupted_data = std::vector<uint8_t>(valid_tables_data.begin(), valid_tables_data.begin() + 50);
