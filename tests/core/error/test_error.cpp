...
TEST_SUITE("error handling") {
    ...
    TEST_CASE("Error category name and messages") {
        ...
        CHECK(category.message(static_cast<int>(errc::io_failure)) == "Input/output operation failed");
        CHECK(category.message(static_cast<int>(errc::invalid_art)) == "Invalid or corrupted ART file format");
        CHECK(category.message(static_cast<int>(errc::invalid_palette)) == "Invalid or corrupted palette file format");
        CHECK(category.message(static_cast<int>(errc::conversion_failure)) == "Color conversion or pixel transformation failed");
        CHECK(category.message(static_cast<int>(errc::encoding_failure)) == "Image encoding operation failed");
        CHECK(category.message(static_cast<int>(errc::unsupported)) == "Requested operation or format is not supported");
    ...
    TEST_CASE("make_error_expected functions") {
        ...
        CHECK(result3.error().code.value() == static_cast<int>(errc::conversion_failure));
