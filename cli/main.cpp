...
std::expected<ImageFormat, std::string> parse_format(const std::string& format_str) {
    ...
    if (lower_format == "png") return ImageFormat::png;
    if (lower_format == "tga") return ImageFormat::tga;
    if (lower_format == "bmp") return ImageFormat::bmp;
    
    return std::unexpected("Unsupported format: " + format_str + ". Supported formats: png, tga, bmp");
}
...
int process_art_file(const CliConfig& config, ProgressTracker& progress) {
    ...
    if (config.verbose) {
        std::cout << std::format("Processing {} tiles from {}...", 
                               art_result->tiles.size(), input_path.filename().string()) << std::endl;
    }
    ...
    if (config.parallel && config.jobs != 1) {
            ...
            threads.emplace_back([&]() {
                while (true) {
                    size_t tile_index;
                    {
                        ...
                        if (current_tile >= art_result->tiles.size()) break;
                        tile_index = current_tile++;
                    }
                    
                    const auto& tile = art_result->tiles[tile_index];
                    std::string output_filename = std::format("{}_{:04d}.{}", base_name, tile_index, image_format_to_string(output_format));
                    std::filesystem::path output_path = std::filesystem::path(config.output_dir) / output_filename;
                    ...
                    if (result) {
                        ...
                        if (!config.quiet) {
                            ...
                            std::cerr << std::format("\nError processing tile {}: {}", tile_index, result.error().message) << std::endl;
                        }
                    }
        ...
        for (size_t i = 0; i < art_result->tiles.size(); ++i) {
            ...
            std::string output_filename = std::format("{}_{:04d}.{}", base_name, i, image_format_to_string(output_format));
            std::filesystem::path output_path = std::filesystem::path(config.output_dir) / output_filename;
            ...
            if (result) {
                ...
                progress.update(0, 1);
                if (!config.quiet) {
                    std::cerr << std::format("Error processing tile {}: {}", i, result.error().message) << std::endl;
                }
            }
...
int process_art_directory(const CliConfig& config) {
    ...
    if (config.verbose) {
        std::cout << std::format("Found {} ART files in directory: {}", 
                               total_progress.total, config.input_path) << std::endl;
    }
    ...
    for (const auto& entry : std::filesystem::directory_iterator(config.input_path)) {
        if (entry.is_regular_file() && entry.path().extension() == ".ART") {
            ...
            if (config.verbose) {
                std::cout << std::format("\nProcessing file {}/{}: {}", 
                                       ++processed, total_progress.total, entry.path().filename().string()) << std::endl;
            }
            ...
            if (!config.quiet) {
                std::cout << std::format("File {}: {} tiles completed, {} tiles failed", 
                                       ...
                                       file_progress.completed, file_progress.failed) << std::endl;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (config.palette_file.empty()) {
        ...
        if (std::filesystem::exists(default_palette)) {
            ...
            std::cerr << "Warning: No palette file specified and PALETTE.DAT not found in input directory" << std::endl;
    ...
    if (std::filesystem::is_directory(config.input_path)) {
        ...
        if (!config.quiet) {
            std::cout << std::format("Conversion complete: {} tiles processed, {} tiles failed", 
                                   progress.completed, progress.failed) << std::endl;
        }
    }
