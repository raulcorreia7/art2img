# art2img v1.1 – Public API Architecture
...
include/
  art2img/
    ...
    adapters/
      ...
      grp.hpp                    // GrpCatalog, load_grp(), find_entry()
...
std::expected<GrpCatalog, core::Error>
load_grp(std::span<const std::byte> blob) noexcept;

std::optional<GrpEntry>
find_entry(const GrpCatalog&, std::string_view name) noexcept;
