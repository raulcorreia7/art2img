...
// It recreates the original API surface while forwarding to the new vNext
// modules
...
public:
  explicit ArtException(const std::string &message)
...
public:
  struct Options {
    ...
    bool premultiply_alpha =
        false; // Apply premultiplication for upscaling (PNG only)
    bool matte_hygiene =
        false; // Apply alpha matte hygiene (erode + blur) (PNG only)
    bool fix_transparency =
        true; // Enable magenta transparency processing (PNG only)

    Options() {}
  };

  // Write image to file
  static bool write_image(const std::filesystem::path &filename,
  ...
  static bool
  write_image_to_memory(std::vector<uint8_t> &output, LegacyImageFormat format,
...
struct ArtView {
  ...
  uint32_t get_start_tile_index() const { return header.start_tile; }
  uint32_t get_end_tile_index() const { return header.end_tile; }
  uint32_t get_num_tiles() const { return header.num_tiles; }
};
...
struct ImageView {
  ...
  bool
  save_to_image(const std::filesystem::path &path, LegacyImageFormat format,
  ...
  std::vector<uint8_t>
  extract_to_image(LegacyImageFormat format,
                   ImageWriter::Options options = ImageWriter::Options()) const;
  std::vector<uint8_t>
  extract_to_png(ImageWriter::Options options = ImageWriter::Options()) const;
...
public:
  ...
  // Extraction methods
  ExtractionResult
  extract_tile(uint32_t tile_index, LegacyImageFormat format,
               ImageWriter::Options options = ImageWriter::Options());
  ExtractionResult
  extract_tile_png(uint32_t tile_index,
                   ImageWriter::Options options = ImageWriter::Options());
  ExtractionResult
  extract_tile_tga(uint32_t tile_index,
                   ImageWriter::Options options = ImageWriter::Options());
  ExtractionResult
  extract_tile_bmp(uint32_t tile_index,
                   ImageWriter::Options options = ImageWriter::Options());

  // Batch extraction
  std::vector<ExtractionResult>
  extract_all_tiles(LegacyImageFormat format,
                    ImageWriter::Options options = ImageWriter::Options());
  std::vector<ExtractionResult>
  extract_all_tiles_png(ImageWriter::Options options = ImageWriter::Options());
  std::vector<ExtractionResult>
  extract_all_tiles_tga(ImageWriter::Options options = ImageWriter::Options());
  std::vector<ExtractionResult>
  extract_all_tiles_bmp(ImageWriter::Options options = ImageWriter::Options());
  ...
  bool write_animation_data(const std::string &art_file_path,
                            const std::string &output_dir) const;
  std::string
  generate_animation_ini_content(const std::string &art_file_path) const;
