...
const ArtFile::Header &ArtFile::header() const { return legacy_header_; }

const std::vector<ArtFile::Tile> &ArtFile::tiles() const {
  return legacy_tiles_;
}

bool ArtFile::is_open() const { return is_open_; }

const std::filesystem::path &ArtFile::filename() const { return filename_; }
...
Palette::Palette()
    : palette_(std::make_unique<::art2img::Palette>()), loaded_(false) {}
...
const std::vector<uint8_t> &Palette::raw_data() const { return raw_data_; }

bool Palette::is_loaded() const { return loaded_; }
...
bool ImageWriter::write_image(const std::filesystem::path &filename,
                              LegacyImageFormat format, const Palette &palette,
  ...
  // Encode the image
  auto encode_result =
      encode_image(image_view(image_result.value()), new_format);
  if (!encode_result) {
    return false;
  }

  // Write to file
  auto byte_span = std::span<const byte>(encode_result.value().data(),
                                         encode_result.value().size());
  auto write_result = write_binary_file(filename, byte_span);
  return write_result.has_value();
}

bool ImageWriter::write_image_to_memory(
    std::vector<uint8_t> &output, LegacyImageFormat format,
    const Palette &palette, const ArtFile::Tile &tile,
    const uint8_t *pixel_data, size_t pixel_data_size, const Options &options) {
  // Convert legacy format to new format
  ::art2img::ImageFormat new_format;
  switch (format) {
  case LegacyImageFormat::PNG:
...
uint32_t ImageView::anim_frames() const { return require_tile().anim_frames(); }

uint32_t ImageView::anim_type() const { return require_tile().anim_type(); }

int8_t ImageView::x_offset() const { return require_tile().x_offset(); }

int8_t ImageView::y_offset() const { return require_tile().y_offset(); }

uint32_t ImageView::anim_speed() const { return require_tile().anim_speed(); }

uint32_t ImageView::other_flags() const { return require_tile().other_flags(); }
...
bool ImageView::save_to_image(const std::filesystem::path &path,
  ...
  return ImageWriter::write_image(path, format, *parent->palette, tile,
...
std::vector<uint8_t>
ImageView::extract_to_image(LegacyImageFormat format,
                            ImageWriter::Options options) const {
  if (!parent || !parent->palette) {
    return {};
  }
  ...
  if (ImageWriter::write_image_to_memory(output, format, *parent->palette, tile,
                                         pixel_data, size(), options)) {
    return output;
  }
  return {};
...
std::vector<uint8_t>
ImageView::extract_to_png(ImageWriter::Options options) const {
...
ExtractionResult ExtractorAPI::extract_tile(uint32_t tile_index,
  ...
  auto encode_result =
      encode_image(image_view(image_result.value()), new_format);
  return create_extraction_result(encode_result, tile_index, format_str,
                                  tile_view);
}
...
ExtractorAPI::extract_all_tiles(LegacyImageFormat format,
                                ImageWriter::Options options) {
...
std::vector<ExtractionResult>
ExtractorAPI::extract_all_tiles_png(ImageWriter::Options options) {
  return extract_all_tiles(LegacyImageFormat::PNG, options);
}

std::vector<ExtractionResult>
ExtractorAPI::extract_all_tiles_tga(ImageWriter::Options options) {
  return extract_all_tiles(LegacyImageFormat::TGA, options);
}

std::vector<ExtractionResult>
ExtractorAPI::extract_all_tiles_bmp(ImageWriter::Options options) {
...
std::string ExtractorAPI::generate_animation_ini_content(
    const std::string &art_file_path) const {
  ...
  for (uint32_t i = 0; i < tile_count; ++i) {
    ...
    if (tile.anim_frames() > 1) {
      ...
      ini << "tile" << i
          << "_type=" << get_animation_type_string(tile.anim_type()) << "\n";
...
ExtractionResult ExtractorAPI::create_extraction_result(
    const std::expected<std::vector<byte>, Error> &result, uint32_t tile_index,
    const std::string &format, const TileView &tile_view) const {
