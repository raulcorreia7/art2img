/// @file convert.cpp
/// @brief Implementation of tile to RGBA conversion functions for Build engine
/// ART files
///
/// This module implements the conversion functionality as specified in
/// Architecture §4.4 and §9. It handles:
...
///
/// Conversion pipeline follows: remaps -> shade -> palette conversion ->
/// transparency fix -> premultiply
///
    ...
    if (options.premultiply_alpha) {
      for (std::size_t i = 0; i < result.data.size(); i += 4) {
        ...
        if (alpha == 0) {
          ...
          result.data[i + 0] =
              static_cast<u8>((result.data[i + 0] * alpha + 127) / 255);
          result.data[i + 1] =
              static_cast<u8>((result.data[i + 1] * alpha + 127) / 255);
          result.data[i + 2] =
              static_cast<u8>((result.data[i + 2] * alpha + 127) / 255);
        }
      }
    }
  }
...
std::expected<u8, Error> get_pixel_column_major(const TileView& tile, u16 x,
  ...
  const std::size_t linear_index =
      static_cast<std::size_t>(x) * tile.height + static_cast<std::size_t>(y);
