...
std::string normalise_name(std::string_view name) {
  ...
  while (!lowered.empty() && (lowered.back() == '\0' || lowered.back() == ' ')) {
    lowered.pop_back();
  }
...
std::expected<GrpFile, core::Error> load_grp(
  ...
  if (blob.size() < kSignature.size() + 4) {
    return std::unexpected(core::make_error(
        core::errc::invalid_art, "blob too small for GRP header"));
  }
  ...
  if (blob.size() < directory_offset + directory_bytes) {
    return std::unexpected(core::make_error(core::errc::invalid_art,
                                            "GRP directory truncated"));
  }

  auto storage = std::make_shared<std::vector<std::byte>>(blob.begin(), blob.end());
  ...
  for (std::uint32_t i = 0; i < entry_count; ++i) {
    std::string name(reinterpret_cast<const char*>(data.data() + directory_cursor),
                     kNameSize);
