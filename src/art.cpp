...
std::uint32_t read_u32_le(std::span<const std::byte> data, std::size_t offset) {
...
constexpr bool is_valid_tile_dimensions(std::uint16_t width, std::uint16_t height) noexcept {
...
std::size_t calculate_total_pixel_size(
    std::span<const std::uint16_t> widths,
...
bool validate_header_consistency(
    ...
    std::uint32_t localtilestart,
    std::uint32_t localtileend) {
    ...
    const std::uint32_t actual_tile_count = localtileend - localtilestart + 1;
...
TileAnimation::TileAnimation(std::uint32_t picanm) 
    : frame_count(static_cast<std::uint8_t>(picanm & 0x3F)),
      type(static_cast<Type>((picanm >> 6) & 0x03)),
      speed(static_cast<std::uint8_t>((picanm >> 24) & 0x0F)),
      x_center_offset(static_cast<std::int8_t>((picanm >> 16) & 0xFF)),
      y_center_offset(static_cast<std::int8_t>((picanm >> 8) & 0xFF)) {
}

std::uint32_t TileAnimation::to_picanm() const noexcept {
    std::uint32_t result = 0;
    result |= static_cast<std::uint32_t>(frame_count & 0x3F);
    result |= static_cast<std::uint32_t>(static_cast<std::uint8_t>(type) & 0x03) << 6;
    result |= static_cast<std::uint32_t>(static_cast<std::uint8_t>(y_center_offset) & 0xFF) << 8;
    result |= static_cast<std::uint32_t>(static_cast<std::uint8_t>(x_center_offset) & 0xFF) << 16;
    result |= static_cast<std::uint32_t>(speed & 0x0F) << 24;
    return result;
}
...
std::optional<TileView> ArtData::get_tile_by_id(std::uint32_t tile_id) const noexcept {
...
std::expected<ArtData, Error> load_art_bundle(
    ...
    const std::uint32_t numtiles = read_u32_le(data, offset);
    offset += 4;
    ...
    const std::uint32_t tile_count = art_data.tile_end - art_data.tile_start + 1;
    ...
    // Parse tile arrays
    std::vector<std::uint16_t> tile_widths(tile_count);
    std::vector<std::uint16_t> tile_heights(tile_count);
    std::vector<std::uint32_t> tile_picanms(tile_count);
...
std::expected<std::vector<std::uint8_t>, Error> load_lookup_data(
    ...
    if (!file) {
        return make_error_expected<std::vector<std::uint8_t>>(errc::io_failure,
    ...
    if (file_size < 256) {
        return make_error_expected<std::vector<std::uint8_t>>(errc::invalid_art,
