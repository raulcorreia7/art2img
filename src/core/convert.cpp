...
std::uint8_t apply_lookup(std::uint8_t index, const TileView& tile,
...
std::uint8_t apply_shade(std::uint8_t index, PaletteView palette,
                         const ConversionOptions& options) noexcept {
  ...
  const std::size_t offset = static_cast<std::size_t>(shade) * shade_table_size +
...
RgbaPixel sample_color(std::uint8_t index, PaletteView palette,
...
void premultiply_alpha(std::vector<std::uint8_t>& pixels) {
  for (std::size_t i = 0; i + 3 < pixels.size(); i += kChannels) {
    ...
    if (alpha == 0) {
      ...
      pixels[i + 0] = static_cast<std::uint8_t>((pixels[i + 0] * alpha + 127) /
                                                255);
      pixels[i + 1] = static_cast<std::uint8_t>((pixels[i + 1] * alpha + 127) /
                                                255);
      pixels[i + 2] = static_cast<std::uint8_t>((pixels[i + 2] * alpha + 127) /
                                                255);
    }
  }
}
...
void apply_matte(std::vector<std::uint8_t>& pixels, std::uint32_t width,
  ...
  for (std::uint32_t y = 0; y < height; ++y) {
    for (std::uint32_t x = 0; x < width; ++x) {
      ...
      if (dst < pixels.size()) {
        pixels[dst] =
            blurred[static_cast<std::size_t>(y) * width + static_cast<std::size_t>(x)];
      }
    }
  }
}
...
std::expected<RgbaImage, Error> palette_to_rgba(
  ...
  if (palette_view.rgb.size() < palette_component_count) {
    return std::unexpected(make_error(errc::invalid_palette,
                                      "palette view missing color data"));
  }
  ...
  const std::size_t row_stride = static_cast<std::size_t>(image.width) * kChannels;
  for (std::uint32_t y = 0; y < tile.height; ++y) {
    for (std::uint32_t x = 0; x < tile.width; ++x) {
      const std::size_t src_index = static_cast<std::size_t>(x) * tile.height + y;
      const auto palette_index = std::to_integer<std::uint8_t>(tile.indices[src_index]);
      const auto color = sample_color(palette_index, palette_view, options, tile);
  ...
  if (options.apply_lookup || options.shade_index.has_value()) {
    // Lookup tables may introduce transparent pixels that need cleanup.
