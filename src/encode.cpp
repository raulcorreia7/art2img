...
///
/// This module implements the image encoding functionality as specified in Architecture §4.5 and §9.
/// It handles:
...
namespace art2img
{
    ...
    namespace
    {
        ...
        {
            const std::size_t row_bytes = static_cast<std::size_t>(image.width) * constants::RGBA_BYTES_PER_PIXEL;
            std::vector<u8> contiguous(static_cast<std::size_t>(image.height) * row_bytes);
        ...
        std::vector<u8> flip_image_vertically(const u8* data,
                                              u16 width,
                                              u16 height,
                                              int channels)
        {
            ...
            for (u16 y = 0; y < height; ++y)
            {
                ...
                const std::size_t src_row = static_cast<std::size_t>(height - 1 - y) * row_bytes;
        ...
        bool validate_image_dimensions(const ImageView& image)
        {
            ...
            const std::size_t expected_stride = static_cast<std::size_t>(image.width) * constants::RGBA_BYTES_PER_PIXEL;
            ...
            // Check data size
            const std::size_t expected_size = image.stride * static_cast<std::size_t>(image.height);
        ...
        const PngOptions& options)
    {
        ...
        {
            return make_error_expected<std::vector<byte>>(errc::encoding_failure,
        ...
        const std::size_t expected_row_bytes = static_cast<std::size_t>(image.width) * constants::RGBA_BYTES_PER_PIXEL;
        ...
        if (options.convert_to_grayscale)
        {
            ...
            for (std::size_t i = 0; i < image.pixel_count(); ++i)
            {
                ...
                grayscale_data[i] = static_cast<u8>((299u * r + 587u * g + 114u * b) / 1000u);
            }   
        ...
        {
            return make_error_expected<std::vector<byte>>(errc::encoding_failure,
                                                         "PNG encoding failed: stb_image_write error");
        }
        ...
        const TgaOptions& options)
    {
        ...
        {
            auto flipped = flip_image_vertically(pixel_data.data(), image.width, image.height, channels);
        ...
        {
            return make_error_expected<std::vector<byte>>(errc::encoding_failure,
                                                         "TGA encoding failed: stb_image_write error");
        }
        ...
        const BmpOptions& options)
    {
        ...
        {
            return make_error_expected<std::vector<byte>>(errc::encoding_failure,
                                                         "BMP encoding failed: stb_image_write error");
        }
        ...
        EncodeOptions options)
    {
        ...
        return make_error_expected<std::vector<byte>>(errc::encoding_failure,
                                                     "Invalid options for image format");
    }
    ...
    std::expected<std::monostate, Error> validate_image_for_encoding(const ImageView& image)
    {
        if (!validate_image_dimensions(image))
        {
            return make_error_expected<std::monostate>(errc::encoding_failure,
