...
///
/// This module implements the file I/O functionality as specified in Architecture §4.5 and §9.
/// It handles:
...
namespace art2img
{
    ...
    std::expected<std::vector<types::byte>, Error> read_binary_file(const std::filesystem::path& path)
    {
        // Open file in binary mode
        std::ifstream file(path, std::ios::binary);
        if (!file)
        {
            return make_error_expected<std::vector<types::byte>>(errc::io_failure,
        ...
        {
            return make_error_expected<std::vector<types::byte>>(errc::io_failure,
                                                         "Failed to seek to end of file: " + path.string());
        }
        ...
        std::span<const types::byte> data)
    {
        ...
        {
            return make_error_expected<std::monostate>(errc::io_failure,
                                                       "Failed to flush file: " + path.string());
        }
    ...
    std::expected<std::monostate, Error> check_file_readable(const std::filesystem::path& path)
    {
        std::error_code ec;
        if (!std::filesystem::exists(path, ec))
        {
            if (ec)
            {
                return make_error_expected<std::monostate>(ec,
                                                           "File access error: " + path.string());
            }
            return make_error_expected<std::monostate>(errc::io_failure,
        ...
        if (!std::filesystem::is_regular_file(path, ec))
        {
            ...
            {
                return make_error_expected<std::monostate>(ec,
                                                           "File type check error: " + path.string());
            }
            return make_error_expected<std::monostate>(errc::io_failure,
        ...
        if (!file)
        {
            const std::error_code file_ec = std::error_code(errno, std::system_category());
            return make_error_expected<std::monostate>(file_ec,
                                                       "File is not readable: " + path.string());
        }

        return make_success();
    }

    std::expected<std::monostate, Error> check_directory_writable(const std::filesystem::path& path)
    {
        std::error_code ec;
        ...
        if (!std::filesystem::exists(path, ec))
        {
            ...
            {
                return make_error_expected<std::monostate>(ec,
                                                           "Directory access error: " + path.string());
            }
        ...
        if (!std::filesystem::is_directory(path, ec))
        {
            ...
            {
                return make_error_expected<std::monostate>(ec,
                                                           "Directory type check error: " + path.string());
            }
            return make_error_expected<std::monostate>(errc::io_failure,
        ...
        if (!file)
        {
            const std::error_code file_ec = std::error_code(errno, std::system_category());
            return make_error_expected<std::monostate>(file_ec,
                                                       "Directory is not writable: " + path.string());
        }
    ...
    std::expected<std::monostate, Error> ensure_directory_exists(const std::filesystem::path& path)
    {
        std::error_code ec;
        if (!std::filesystem::exists(path, ec))
        {
        ...
        else if (!std::filesystem::is_directory(path, ec))
        {
            ...
            {
                return make_error_expected<std::monostate>(ec,
                                                           "Path type check error: " + path.string());
            }
            return make_error_expected<std::monostate>(errc::io_failure,
    ...
    std::expected<std::size_t, Error> get_file_size(const std::filesystem::path& path)
    {
        std::error_code ec;
        const auto file_size = std::filesystem::file_size(path, ec);
        if (ec)
        {
            return make_error_expected<std::size_t>(errc::io_failure,
    ...
    std::expected<std::string, Error> read_text_file(const std::filesystem::path& path)
    {
        ...
        if (!file)
        {
            ...
            return make_error_expected<std::string>(ec,
                                                   "Failed to open text file for reading: " + path.string());
        }
        ...
        {
            return make_error_expected<std::string>(errc::io_failure,
                                                   "Failed to read text file: " + path.string() + 
                                                   " (" + e.what() + ")");
        }
    }
        ...
        const std::string& content)
    {
        ...
        if (!file)
        {
            const std::error_code ec = std::error_code(errno, std::system_category());
            return make_error_expected<std::monostate>(ec,
                                                       "Failed to open text file for writing: " + path.string());
        }
        ...
        if (!file)
        {
            const std::error_code ec = std::error_code(errno, std::system_category());
            return make_error_expected<std::monostate>(ec,
                                                       "Failed to write to text file: " + path.string());
        }
        ...
        if (!file.flush())
        {
            const std::error_code ec = std::error_code(errno, std::system_category());
            return make_error_expected<std::monostate>(ec,
                                                       "Failed to flush text file: " + path.string());
        }
