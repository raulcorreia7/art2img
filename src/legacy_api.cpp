...
void Palette::load_build_engine_default() {
    // For now, create a basic palette - this would need actual default palette data
...
void Palette::load_duke3d_default() {
    // For now, create a basic palette - this would need actual Duke3D palette data
    palette_ = std::make_unique<::art2img::Palette>();
    loaded_ = true;
    update_raw_data();
}
...
bool ImageWriter::write_image(const std::filesystem::path& filename, ImageFormat format,
    ...
    // Write to file
    auto byte_span = std::span<const byte>(encode_result.value().data(), encode_result.value().size());
...
bool ImageView::save_to_image(const std::filesystem::path& path, ImageFormat format,
    ...
    return ImageWriter::write_image(path, format, *parent->palette, tile, pixel_data, size(), options);
}
...
std::vector<uint8_t> ImageView::extract_to_image(
    ...
    if (ImageWriter::write_image_to_memory(output, format, *parent->palette, tile, pixel_data, size(), options)) {
        return output;
    }
    return {};
}
...
bool ExtractorAPI::write_animation_data(const std::string& art_file_path, const std::string& output_dir) const {
