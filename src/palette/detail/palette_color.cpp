...
color::Color make_palette_color(const Palette& palette, types::u8 index) noexcept
{
    ...
    const std::size_t base = static_cast<std::size_t>(index) * constants::COLOR_COMPONENTS;

    return color::make_rgba(
        scale_6bit_to_8bit(palette.data[base]),
